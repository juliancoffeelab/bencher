...
def run_app():
    ...
    with dpg.window(label="Dashboard", tag="main_window"):
        dpg.add_text("Automated GPU Plotting (Dual Series)")
        dpg.add_separator()
    ...
    while dpg.is_dearpygui_running():
        # Update both active series every frame with a phase shift
        y_offset = frame * 0.1
        for idx, tag in enumerate(active_series):
            # Applying a unique phase shift to each plot for visual distinction
            y_data = (np.sin(np.array(x_data) + y_offset + (idx * np.pi / 2))).tolist()
